...
mod tests {
	...
	fn can_get_state_call_metadata_now() {
		...
		let payload =
			async_std::task::block_on(parachain.query_state_call("Metadata_metadata", &key[..], None)).unwrap();

		let payload2 =
			async_std::task::block_on(parachain.query_metadata(None)).unwrap();
