...
pub struct CpuRateLimiter<T> {
    ...
    #[pin]
    prevent_poll_until: crate::smol::timers::Delay,
}
...
impl<T> CpuRateLimiter<T> {
    ...
    pub fn new(inner: T, rate_limit: u32) -> Self {
        ...
        CpuRateLimiter {
            ...
            prevent_poll_until: crate::smol::timers::Delay::new(Duration::new(0, 0)),
        }
    }
}
...
impl<T: Future> Future for CpuRateLimiter<T> {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context) -> Poll<Self::Output> {
        ...
        let before_polling = crate::smol::Instant::now();

        match this.inner.poll(cx) {
            Poll::Ready(value) => return Poll::Ready(value),
            Poll::Pending => {
                let after_polling = crate::smol::Instant::now();
                ...
                this.prevent_poll_until.set(crate::smol::timers::Delay::new_at(
