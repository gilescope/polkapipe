...
use crate::smol::{alloc2, bindings, cpu_rate_limiter, platform, timers::Delay};
        ...
        .unbounded_send((
            ...
            Box::pin(async move {
                ...
                loop {
                    ...
                    let mem = u64::try_from(alloc2::total_alloc_bytes()).unwrap();
...
fn spawn_background_task(future: impl Future<Output = ()> + Send + 'static) {
    ...
    impl task::Wake for Waker {
        fn wake(self: Arc<Self>) {
            ...
            crate::smol::start_timer_wrap(Duration::new(0, 0), move || {
